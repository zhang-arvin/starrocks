...
>>>>>>> 0d427837fb ([BugFix] Keep array_sort's lambda comparator from driving the sort out of bounds (#77687))
...
StatusOr<ColumnPtr> ArraySortLambdaExpr::evaluate_lambda_expr(ExprContext* context, Chunk* chunk,
    ...
    if (data_column->size() == num_rows) {
        ...
        auto array_size = offsets_col.get_data()[1] - offsets_col.get_data()[0];
    ...
    for (size_t row = 0; row < num_rows; ++row) {
        ...
        auto start_idx = offsets_col.get_data()[array_elem_idx];
        auto end_idx = offsets_col.get_data()[array_elem_idx + 1];
...
StatusOr<ColumnPtr> ArraySortLambdaExpr::evaluate_checked(ExprContext* context, Chunk* chunk) {
    ...
    if (array_col->is_nullable()) {
        ...
        data_column->empty_null_in_complex_column(null_column->get_data(), offsets.get_data());
    }
